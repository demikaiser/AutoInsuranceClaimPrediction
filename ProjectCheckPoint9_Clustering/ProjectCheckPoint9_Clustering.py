...
class DataScienceModeler:
    ...
    def load_trainingset(self, shuffle, PARAMETER):
        ...
        self.c_y_train = np.where(self.r_y_train > PARAMETER, 1, 0)
    ...
    def experiment03(self):
        ...
        for o in range(10, 300, 10):
            self.load_trainingset(False, o)
            ...
            model_classification_1.fit(self.c_x_train, self.c_y_train)
            # y_intermediate = model_classification_1.predict_proba(self.c_x_train)[:, 1]
            # y_intermediate = np.where(y_intermediate > 0.7, 1, 0)
            y_intermediate = model_classification_1.predict(self.c_x_train)

            self.print_classification_performance_metrics(self.c_y_train, y_intermediate)

            model_classification_2 = xgboost.XGBClassifier(learning_rate=0.1, n_estimators=100,
                                      max_depth=30, min_child_weight=1, gamma=0.1,
                                      subsample=0.8, colsample_bytree=0.8, colsample_bylevel=1,
                                      objective='binary:logistic', nthread=4, n_jobs=2,
                                      booster='gbtree', scale_pos_weight=20,
            ...
            model_classification_2.fit(self.c_x_train, y_intermediate)
            ...
            y_pred_prob = model_classification_2.predict_proba(self.c_x_test)[:, 1]

            for TAU in TAU_LIST:
                # self.log("---- ---- ---- ---- TAU:" + str(TAU) + " ---- ---- ---- ----")
                y_pred_classification = np.where(y_pred_prob > TAU, 1, 0)
                f1_current = self.print_classification_performance_metrics(self.c_y_test, y_pred_classification)
                if f1_current > f1_score_best:
                    f1_score_best = f1_current
                    tau_best = TAU
